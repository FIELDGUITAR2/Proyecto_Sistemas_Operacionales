...
void resaltar_linea_sin_regex(Linea *linea, SyntaxRule *rule);
...
// --- Carga de configuración de sintaxis (sin regex) ---
...
void load_syntax_config() {
    ...
    // Si no existe, crear reglas por defecto para C
    if (!fp) {
        // Regla para C
        SyntaxRule *r = &E.syntax_rules[E.syntax_rule_count++];
        strcpy(r->ext, "c");
        // Palabras reservadas básicas de C
        const char *def_kw[] = {"int", "char", "void", "return", "if", "else", "for", "while",
                                "struct", "typedef", "static", "include", "define"};
        const char *def_col[] = {"31", "34", "35", "36", "33", "33", "32", "32",
                                 "35", "35", "35", "33", "33"};
        int n = sizeof(def_kw)/sizeof(def_kw[0]);
        r->count = n;
        r->keywords = malloc(n * sizeof(char*));
        r->colors   = malloc(n * sizeof(char*));
        for (int i = 0; i < n; i++) {
            r->keywords[i] = strdup(def_kw[i]);
            r->colors[i]   = strdup(def_col[i]);
        }
        // También para cabeceras .h
...
// --- Resaltado sin expresiones regulares ---
// Recorre la línea carácter a carácter, detecta palabras (letras, dígitos, '_')
// y las compara con el diccionario. Si coincide, añade color ANSI.
...
void resaltar_linea_sin_regex(Linea *linea, SyntaxRule *rule) {
    ...
    // Buffer de salida: tamaño suficiente (cada palabra puede crecer con códigos)
    ...
    int i = 0;
    while (i < (int)len) {
        // Saltar caracteres no alfabéticos (excepto '_' y dígitos)
        ...
        if (word_len < 64) {
            ...
            // Palabra demasiado larga, copiar tal cual
            for (int j = start; j < i; j++) out[pos++] = text[j];
            continue;
        }
        ...
        for (int k = 0; k < rule->count; k++) {
            if (strcmp(word, rule->keywords[k]) == 0) {
                color = rule->colors[k];
                break;
            }
        }
        ...
        if (color != NULL) {
            ...
            // Agregar la palabra
            pos += snprintf(out + pos, cap - pos, "%s", word);
            // Resetear color
...
void *syntax_thread(void *arg) {
    ...
    while (E.running) {
        ...
        if (rule) {
            ...
            while (l != NULL) {
                resaltar_linea_sin_regex(l, rule);
                l = l->siguiente;
            }
        }
        pthread_mutex_unlock(&E.mutex_buffer);
...
void borrar_caracter_izquierda() {
    ...
    if (E.cx == 0) {
        ...
        if (E.linea_actual->anterior == NULL) return;
...
// --- Procesamiento de teclas (con flechas, home, end, backspace, delete) ---
